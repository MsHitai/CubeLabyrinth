...
public class MazeGenerator
{
    ...
    private void RemoveWallsWithBackTracker(MazeGeneratorCell[,] maze)
    {
        ...
        do
        {
            ...
            if (x < Width - 2 && !maze[x + 1, y].Visited) unvisitedNeighbors.Add(maze[x + 1, y]);
            if (y < Height - 2 && !maze[x, y + 1].Visited) unvisitedNeighbors.Add(maze[x, y + 1]);

            if (unvisitedNeighbors.Count > 0)
            {
                MazeGeneratorCell chosen = unvisitedNeighbors[UnityEngine.Random.Range(0, unvisitedNeighbors.Count)];
